...
class Bezier:
    def TwoPoints(t, P1, P2):
        ...
        if not isinstance(P1, np.ndarray) or not isinstance(
            P2, np.ndarray
        ):
            raise TypeError(
                "Points must be an instance of the numpy.ndarray!"
            )
        if not isinstance(t, (int, float)):
            raise TypeError(
                "Parameter t must be an int or float!"
            )
    ...
    def Curve(t_values, points):
        ...
        for t in t_values:
            ...
            curve = np.append(
                curve, [Bezier.Point(t, points)], axis=0
            )
